...
class DateParser:
    ...
    def __init__(self, verbose=True):
        ...
        self._created_datetime_cols = {} #save created datetime columns for transform method
        self.dayfirst = {} #save dayfirst parameter for each datetime column for transform method
    ...
    def col_contains_dates(self, series):
        ...
        return all(series.str.contains(r'\d{1,4}.\d{1,2}.\d{1,4}', regex=True, na=True))
    ...
    def find_datetime_cols(self, df):
        ...
        if self._datetime_cols is not None:
            self.printer.print(f'Datetime columns found: {self._datetime_cols}', order=2)
        return df
    ...
    def convert_to_datetime(self, dt_str_series):
        ...
        dt_series_parsed = pd.to_datetime(dt_str_series, dayfirst=dayfirst, errors='coerce')
        if np.all(dt_series_parsed.isnull()):
    ...
    def extract_date_features(self, df, col):
        ...
        '''
        features = ['year', 'month', 'day', 'hour', 'minute', 'second', 'weekday', 'quarter']
    ...
    def fit_transform(self, df):
        ...
        try:
            ...
            self.printer.print('Error at DateParser.fit_transform. Returning untransformed df', order='error')
            print(e)
            self._datetime_cols = None
            return df 
        return data
    ...
    def transform(self, df):
        ...
        '''
        if self._datetime_cols is None:
            self.printer.print('No datetime columns were found at fit_transform', order=1)
            return df
        for col in self._datetime_cols:
            df[col] = pd.to_datetime(df[col], dayfirst=self.dayfirst[col], errors='coerce')
            for feature in self._created_datetime_cols[col]:
                if feature not in ['week', 'part_of_day']:
                    df[f'{col}_{feature}'] = self.extract_date_feature(df[col], feature, fit_transform=False)
