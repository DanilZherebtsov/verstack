...
class NaNImputer:
    ...
    def train_sample_size(self, value):
        if type(value) != int:
            raise TypeError("train_sample_size must be an int")
    ...
    def _fill_with_string(self, df, col):
        ...
        self.printer.print(
            f'Missing values in {col} replaced by "Missing_data" string', order=3
        )
        return df
    ...
    def _fill_object_nan_cols_with_string(self, df, is_train=True):
        ...
        """
        object_nan_cols = [
            col for col in df.select_dtypes(include="O") if np.any(df[col].isnull())
        ]
    ...
    def _process_df(self, df):
        ...
        num_cols_to_process = len(object_cols) + len(
            [col for col in self._cols_to_impute if col not in object_cols]
        )
        cnt = 0
        for col in object_cols:
            if col not in self._imputed_with_string:
                ...
                if cnt % 10 == 0:
                    self.printer.print(
                        f"Processed {cnt} cols; {num_cols_to_process - cnt} to go",
                        order=3,
                    )
    ...
    def _sample_data(self, df, col):
        ...
        if objective in ["binary", "multiclass"]:
            if sample_size < len(df_non_nan):
                train = df_non_nan.groupby(col, group_keys=False).apply(
    ...
    def _get_high_corr_feats(self, df, col):
        ...
        not_supported = (
            df.drop(col, axis=1)
            .select_dtypes(include=["O", "datetime"])
            .columns.tolist()
        )
        corellations = df.drop(exclude_from_df + [col] + not_supported, axis=1).apply(
            lambda x: x.corr(df[col])
        )
    ...
    def _predict_nan_in_col(self, df, col):
        ...
        try:
            ...
            X_train_col, y_train_col, X_test_col = self._get_splits(
                df, col
            )  # already sampled and with 10 feats
            # clean up
            del df
            gc.collect()
            params = self._select_params(objective, y_train_col)
            pred = self._train_predict_decode(
                params, X_train_col, y_train_col, X_test_col
            )
            self.printer.print(
                f"Imputed ({objective:^10}) - {len(X_test_col):<8} NaN in {col}",
                order=3,
            )
        except:
    ...
    def _impute_single_core(self, df):
        ...
        self.printer.print(
            f"Imputing single core {len(self._cols_to_impute.keys())} cols", order=2
        )
    ...
    def _fill_constants_in_test(self, df):
    ...
    def impute(self, data, train=True):
        ...
        self.printer.print(
            f"Missing values after imputation: {sum(df.isnull().sum())}", order=2
        )
        return df
