...
from verstack.lgbm_optuna_tuning.lgb_metrics import (
    get_pruning_metric,
    get_optimization_metric_func,
    define_objective,
    classification_metrics,
    regression_metrics,
    get_eval_score,
    print_lower_greater_better,
    supported_metrics,
)
from verstack.lgbm_optuna_tuning.args_validators import *
from verstack.lgbm_optuna_tuning.optuna_tools import (
    Distribution,
...
class LGBMTuner:
    ...
    def metric(self, value):
        ...
        if value not in supported_metrics:
            raise KeyError(
                f"LGBMTuner supports the following evaluation metrics: {supported_metrics}"
            )
        self._metric = value
    ...
    def trials(self, value):
        ...
        if value <= 0:
            raise ValueError("trials must be an integer > 0")
        self._trials = value
    ...
    def refit(self, value):
        if not self._is_bool(value):
            raise TypeError("acceptable refit options are True/False")
        self._refit = value
    ...
    def verbosity(self, value):
        ...
        if value not in range(0, 6):
            raise ValueError("acceptable verbosity options are [0,1,2,3,4,5]")
    ...
    def visualization(self, value):
        if not self._is_bool(value):
            raise TypeError("acceptable visualization options are True/False")
    ...
    def device_type(self, value):
        if not value in ["cpu", "gpu", "cuda", "cuda_exp"]:
            raise TypeError("acceptable device_type options are cpu/gpu/cuda/cuda_exp")
    ...
    def feature_importances(self):
        if not self._fitted_model:
            raise AttributeError(
                "LGBMTuner.fit(refit = True) must be applied before feature_importances can be displayed"
            )
    ...
    def eval_results_callback(self, value):
        if not value is None:
            if not hasattr(value, "__call__"):
                raise TypeError("eval_results_callback must be a function")
    ...
    def _init_params_on_input(self, rows_num, y):
        ...
        self._init_params["objective"] = define_objective(
            self.metric, self.target_classes
        )
        self._init_params["metric"] = get_pruning_metric(
            self.metric, self.target_classes
        )
        # -------------------------------------------------------------------------
        ...
        if (
            "scale_pos_weight" in self._init_params
            or "is_unbalance" in self._init_params
        ):
    ...
    def _get_default_search_space(self):
        ...
        search_space["feature_fraction"] = SearchSpace(
            Distribution.UNIFORM, low=0.5, high=1.0
        )
        search_space["num_leaves"] = SearchSpace(
            Distribution.INTUNIFORM, low=16, high=255
        )

        if self.trials > 30:
            search_space["bagging_fraction"] = SearchSpace(
                Distribution.UNIFORM, low=0.5, high=1.0
            )
            search_space["min_sum_hessian_in_leaf"] = SearchSpace(
                Distribution.LOGUNIFORM, low=1e-3, high=10.0
            )

        if self.trials > 100:
            search_space["lambda_l1"] = SearchSpace(
                Distribution.LOGUNIFORM, low=1e-8, high=10.0
            )
            search_space["lambda_l2"] = SearchSpace(
                Distribution.LOGUNIFORM, low=1e-8, high=10.0
            )
    ...
    def _get_dtrain_dvalid_objects(
        self, X, y, metric, seed=None, return_raw_valid=False
    ):
        """
    ...
    def _get_validation_score(
        ...
        """
        gbm = lgb.train(
            params, dtrain, valid_sets=[dvalid], callbacks=[pruning_callback]
        )
        pred = gbm.predict(valid_x)
        if (
            self.metric in classification_metrics
        ):  # make sure the log_loss will not throw an error if stratification did not do a good job
        ...
        self.printer.print(
            f"Optimization score ({optimization_direction:<4}): {optimization_metric_func.__name__}: {result}",
            order=4,
        )
        ...
        if self.metric != optimization_metric_func.__name__:
            ...
            self.printer.print(
                f"Evaluation score ({print_lower_greater_better(self.metric):<4}): {self.metric}: {eval_score}",
                order=4,
            )
            # save evaluation metric results per each trial
            self.eval_results[f"train_trial_{trial.number}_out_of_{self.trials}"] = (
                eval_score
            )
    ...
    def _objective(self, trial, X, y):
        ...
        dtrain, dvalid, valid_x, valid_y = self._get_dtrain_dvalid_objects(
            X, y, self.metric, return_raw_valid=True
        )
        params = self._sample_from_search_space(trial)
        # Add a callback for pruning.
        pruning_callback = optuna.integration.LightGBMPruningCallback(
            trial, get_pruning_metric(self.metric, self.target_classes)
        )
        result = self._get_validation_score(
            trial,
            dtrain,
            dvalid,
            valid_x,
    ...
    def _check_refit_status(self, method):
        if not self.refit:
            raise ModuleNotFoundError(
                f"LGBMTuner.fit(refit = True) must be applied before {method} execution"
            )
    ...
    def predict(self, test, threshold=0.5):
        ...
        pred = self._fitted_model.predict(
            test.values, num_threads=self._init_params["num_threads"]
        )
    ...
    def predict_proba(self, test):
        ...
        if self.metric in regression_metrics:
            raise TypeError(
                "predict_proba() is applicable for classification problems only"
            )
        pred = self._fitted_model.predict(
            test.values, num_threads=self._init_params["num_threads"]
        )
    ...
    def _save_feature_importances(self, train_features):
        ...
        feat_importances = pd.Series(
            self._fitted_model.feature_importance(), index=train_features
        )
    ...
    def __configure_matplotlib_style(self, dark=True):
    ...
    def _plot_static_fim(
        self, feat_imp, figsize=(10, 6), dark=True, save=False, display=True
    ):
        """
        ...
        if save:
            ...
            print(
                f'Feature Importance Plot is saved to {os.path.join(os.getcwd(), f"{name}.png")}'
            )

    # ------------------------------------------------------------------------------------------
    def _plot_interactive_fim(
        self,
        plot_df,
        dark=True,
        save=False,
        ...
        default_plotly_dark_fig_update_layout_kwargs = {
            "plot_bgcolor": "#20253c",  # plot color
        ...
        default_plotly_light_fig_update_layout_kwargs = {
            "plot_bgcolor": "#dee0eb",
            "paper_bgcolor": "#fff",
            "font_color": "#34383d",
            "title_font_color": "lightgrey",
        ...
        fig.update_traces(textposition="inside", marker_line_color=BAR_COLOR)
        if display:
            try:
        ...
        if save:
            fig.write_html(name, config={"displaylogo": False})
            print(
                f"Feature Importance Plot is saved to {os.path.join(os.getcwd(), name)}"
            )
    ...
    def plot_importances(
        ...
        if interactive:
            importances_for_html_plot = pd.DataFrame(
                self._feature_importances.nlargest(n_features).sort_values(),
                columns=["importance"],
            )
            ...
            importances_for_html_plot["importance_percent"] = [
                str(np.round(val * 100, 3)) + "%"
                for val in importances_for_html_plot["importance"]
            ]
            self._plot_interactive_fim(
                importances_for_html_plot,
                dark=dark,
                save=save,
                display=display,
            ...
            importances_for_png_plot = self._feature_importances.nlargest(
                n_features
            ).sort_values()
            self._plot_static_fim(
                importances_for_png_plot,
                figsize=figsize,
                dark=dark,
                save=save,
                display=display,
    ...
    def _set_optuna_verbosity(self, value):
        ...
        value_to_optuna_verbosity_dict = {
            0: "CRITICAL",
            1: "CRITICAL",
            2: "ERROR",
            3: "WARNING",
        ...
        optuna.logging.set_verbosity(
            getattr(optuna.logging, value_to_optuna_verbosity_dict[value])
        )
    ...
    def _align_grid_and_search_space(self):
        ...
        for param_name, param_grid in self.grid.items():
            ...
            if isinstance(param_grid, list):
                self.search_space[param_name] = SearchSpace(
                    Distribution.CHOICE, choices=param_grid
                )
            elif isinstance(param_grid, tuple):
                if self._contains_float(param_grid):
                    self.search_space[param_name] = SearchSpace(
                        Distribution.UNIFORM, low=min(param_grid), high=max(param_grid)
                    )
                ...
                if "choices" in param_grid:
                    self.search_space[param_name] = SearchSpace(
                        Distribution.CHOICE, choices=param_grid["choices"]
                    )
        ...
        if unsupported_params:
            self.printer.print(
                f"Following changed parameters are not supported for tuning: {unsupported_params}",
                order="error",
                trailing_blank_paragraph=True,
            )
        self.grid = {
            key: value
    ...
    def fit(self, X, y, optuna_study_params=None):
        ...
        if self.metric == "rmsle" and (y < 0).any():
            self.printer.print(
                "'rmsle' cannot be used when target has negative values, switching to 'rmse'",
                order=1,
            )
            self.metric = "rmse"
        ...
        self.printer.print(
            f"Study direction: minimize {optimization_metric_func.__name__}", order=4
        )
        print()
        ...
        if (
            not "is_unbalance" in self._init_params
            or not "scale_pos_weight" in self._init_params
        ):
            best_trial_number = study.best_trial.number
            num_iterations_in_best_trial = self.early_stopping_results[
                best_trial_number
            ]
        ...
        self.printer.print(
            f"Best trial number:{study.best_trial.number:>2}{break_symbol:>5}     {optimization_metric_func.__name__}:{study.best_trial.value:>29}",
            order=4,
            breakline="-",
        )
